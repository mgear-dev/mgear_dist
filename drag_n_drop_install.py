...
class InstallUI(QtWidgets.QDialog):
    ...
    def create_widgets(self):
        ...
        self.info_label = QtWidgets.QLabel("Make sure to SAVE your scene before continuing.")
        self.install_label = QtWidgets.QLabel("Install Path:")
        self.install_info_label = QtWidgets.QLabel("The path where all the files will be installed.")
    ...
    def create_connections(self):
        ...
        self.install_path_button.clicked.connect(self.on_install_path_button_clicked)
    ...
    def start_install(self):
        ...
        for item in os.listdir(mgear_folder):
            ...
            self.update_logging_widget("Moved: {0}".format(os.path.join(install_path, item)))
        ...
        if install_path != self.get_default_modules_folder():
            if os.path.exists(env_file):
                self.update_logging_widget("Found .env file: {0}".format(env_file))

            self.update_logging_widget("Custom path installation: {0}".format(install_path))
            ...
            if not os.path.join(install_path, "scripts") in sys.path:
                ...
                self.update_logging_widget("Added to PYTHONPATH: {0}".format(os.path.join(install_path, "scripts")))
    ...
    def update_env_file(self, env_file, install_path):
        ...
        if not result:
            self.append_new_line(env_file, "{0}={1}".format(MGEAR_MOD_PATH, install_path))
            self.update_logging_widget("The .env was empty, added {0}.".format(MGEAR_MOD_PATH))

            self.append_new_line(env_file, "{0}=%{1}%;".format(MAYA_MOD_PATH, MGEAR_MOD_PATH))
            self.update_logging_widget("The .env was empty, added {0}.".format(MAYA_MOD_PATH))
            ...
            if not mgear_path:
                ...
                self.pre_append_line(env_file, '{0}="{1}"'.format(MGEAR_MOD_PATH, install_path))
                self.update_logging_widget("Added {0} to .env file.".format(MGEAR_MOD_PATH))
            ...
            if not maya_module_path:
                ...
                maya_module_path = "\n{0}=%{1}%;".format(MAYA_MOD_PATH, MGEAR_MOD_PATH)
                self.append_new_line(env_file, maya_module_path)
                self.update_logging_widget("Added {0} to .env file.".format(MAYA_MOD_PATH))
            else:
                # -- check if mgear module path is there already
                match = re.search(r"(%{0}%)".format(MGEAR_MOD_PATH), str(result))
                if not match:
                    # -- add mgear to module path
                    self.add_to_module_path(env_file, result, MAYA_MOD_PATH, MGEAR_MOD_PATH)
                    self.update_logging_widget("Added {0} to .env file.".format(MGEAR_MOD_PATH))
    ...
    def make_env_backup(self, file_path):
        ...
        self.update_logging_widget("Made backup of .env file: {0}".format(backup_file))
    ...
    def add_to_module_path(self, file_path, paths, module_match, module_add):
        ...
        if old_maya_module_path:
            if old_maya_module_path.endswith("="):
                maya_module_path = "{0}%{1}%".format(old_maya_module_path, module_add)
            elif old_maya_module_path.endswith(";"):
                maya_module_path = "{0}%{1}%".format(old_maya_module_path, module_add)
            else:
                maya_module_path = "{0};%{1}%".format(old_maya_module_path, module_add)

            self.inplace_change(file_path, old_maya_module_path, maya_module_path)
    ...
    def files_exist(self, file_list):
        ...
        for item in file_list:
            if os.path.exists(item):
                ...
                self.update_logging_widget("Found existing file: {0}".format(found))
    ...
    def add_directory(self, source, destination):
        ...
        try:
            ...
            self.update_logging_widget("Copying: {0} to: {1}".format(source, destination))
    ...
    def showEvent(self, event):
        ...
        self.set_line_edit_text(self.install_path_line_edit, self.get_default_modules_folder())
